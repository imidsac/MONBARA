...
echo '<form action="paie_cl_moi.php?&annee='.$annee.'&mois='.$mois.'&bar='.$bar.'&id_bo='.$_GET['id_cl'].'" method="post">';
	...
	echo '<tr>
				<th class="header3 ldroite">DATE</th>
					<td>
					<input type="text" class="text header1 ui-corner-all" name="date_facp" size="20" value="'.$_GET['date_facp'].'" />
					</td>
			</tr>';
	echo '<tr>
...
while ($ligne=pg_fetch_assoc($resultat))
 {
   ...
   echo '</select></td></tr>';*/
	echo '<tr>
				...
				<input type="hidden" name="id_facp" value="'.$_GET['id_facp'].'" />
				<input type=hidden name="role1" value="modag"/>
				</form></td>
				
				<td><a href="paie_cl_moi.php?&annee='.$annee.'&mois='.$mois.'&bar='.$bar.'&id_bo='.$_GET['id_cl'].'">
