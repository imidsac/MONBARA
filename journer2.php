...
<h1 align="center"><strong>LES RETOURS DE <? echo $dater ?></strong></h1>
	
<?php
$resultat1=pg_query($connexion, "SELECT sum(payee) as payee,sum(reste) as reste,sum(somme) as somme 
													from facture 
													where id_bo=$uti
													and extract(year from date_fac)=extract(year from '$_POST[dater]'::date)
...
$resultat2=pg_query($connexion, "SELECT sum(payee) as payee,sum(reste) as reste,sum(somme) as somme 
													from achat 
													where id_bo=$uti
													and extract(year from date_1)=extract(year from '$_POST[dater]'::date)  
													and extract(month from date_1)=extract(month from '$_POST[dater]'::date) 
													and extract(day from date_1)=extract(day from '$_POST[dater]'::date)");
$resultat3=pg_query($connexion, "SELECT sum(montant) as montant 
													from depences 
													where id_bo=$uti
													and extract(year from date_dep)=extract(year from '$_POST[dater]'::date)  
													and extract(month from date_dep)=extract(month from '$_POST[dater]'::date)
													and extract(day from date_dep)=extract(day from '$_POST[dater]'::date)");
...
if($_SESSION['gid'] ==1 || $_SESSION['gid'] ==1000 ) {
	echo '<a href="/html2pdf/pdf/journe_pdfr.php?dater='.$_POST['dater'].'" title="Les retours en format pdf">
...
echo '<tr class="header2 bw">
			...
			<th colspan="3" align="center">SORTIES</th>
			<th colspan="3" align="center">ENTREES</th>
...
echo '<tr class="header2 bw">
     ...
     if($_SESSION['id_bo'] == 1 ) {
			...
			echo '<tr>';
			echo '<td align="center" class="lcentre cnoire">'.number_format($som_a,0,' ',' ').'<sup>F</sup></td>';
			echo '<td align="center" class="lcentre cbleu">'.number_format($som_v,0,' ',' ').'<sup>F</sup></td>';
			echo '<td align="center" class="lcentre crouge">'.number_format($som_d,0,' ',' ').'<sup>F</sup></td>';
...
$lsclient=pg_query($connexion, "select distinct id_cl,nom_cl, prenom_cl 
                                    ...
                                    and extract(day from date_fac)=extract(day from '$_POST[dater]'::date)
                                    and extract(month from date_fac)=extract(month from '$_POST[dater]'::date) 
                                    and extract(year from date_fac)=extract(year from '$_POST[dater]'::date) order by nom_cl, prenom_cl");
...
while ($ligne=pg_fetch_assoc($lsclient))
 ...
 $lscountcl=pg_query($connexion, "select count(*) as nb from (select id_cl,id_ar,prix_vente  
                              ...
                              where extract(day from date_fac)=extract(day from '$_POST[dater]'::date)
                              and extract(month from date_fac)=extract(month from '$_POST[dater]'::date) 
                              and extract(year from date_fac)=extract(year from '$_POST[dater]'::date)  
	...
	$lsachat=pg_query($connexion, "select id_cl,lib_ar, type_ar, sum(qte_ar) as qte_ar,facture_con.prix_vente,sum(qte_ar*facture_con.prix_vente) as montant 
	                           ...
	                           where extract(day from date_fac)=extract(day from '$_POST[dater]'::date)
                              and extract(month from date_fac)=extract(month from '$_POST[dater]'::date)
                              and extract(year from date_fac)=extract(year from '$_POST[dater]'::date) and id_cl=$id_cl 
...
if($_SESSION['id_bo'] == 1 ) {
//========================================AGENCES=======================================================
	echo '<tr class="header3 bw"><th colspan="8"><h5 align="center" class="titre1">ETAT GENERAL DES AGENCES</h5></th></tr>';
      echo '<tr class="header2 lgauche bw"><th>N°</th>
...
$lsagence=pg_query($connexion, "select distinct id_bo,nom_bo 
                                    ...
                                    where  
                                    extract(day from date_tr)=extract(day from '$_POST[dater]'::date) 
                                    and extract(month from date_tr)=extract(month from '$_POST[dater]'::date) 
                                    and extract(year from date_tr)=extract(year from '$_POST[dater]'::date) order by nom_bo");
...
while ($ligne=pg_fetch_assoc($lsagence))
 ...
 $lscountbo=pg_query($connexion, "select count(*) as nb from (select id_bo,id_ar,prix_vente  
                              ...
                              where extract(day from date_tr)=extract(day from '$_POST[dater]'::date) 
                              and extract(month from date_tr)=extract(month from '$_POST[dater]'::date)
                              and extract(year from date_tr)=extract(year from '$_POST[dater]'::date) 
	...
	$lsachat2=pg_query($connexion, "select id_bo,lib_ar, type_ar, sum(qte_ar) as qte_ar,transferts_con.prix_vente,
	                           ...
	                           where extract(day from date_tr)=extract(day from '$_POST[dater]'::date) 
                              and extract(month from date_tr)=extract(month from '$_POST[dater]'::date)
                              and extract(year from date_tr)=extract(year from '$_POST[dater]'::date) and id_bo=$id_bo 
	                           group by  lib_ar,type_ar,transferts_con.prix_vente,id_bo order by lib_ar,type_ar");
...
$resultatcl2=pg_query($connexion, "SELECT facpaiement.id_cl,id_fac,nom_cl,prenom_cl,date_facp,motif,montant from facpaiement join clients using(id_cl) 
													...
													and extract(day from date_facp)=extract(day from '$_POST[dater]'::date)
													and extract(month from date_facp)=extract(month from '$_POST[dater]'::date)
													and extract(year from date_facp)=extract(year from '$_POST[dater]'::date)
													order by nom_cl,prenom_cl
													");
...
$resultatbo2=pg_query($connexion, "SELECT id_tr,id_bo,nom_bo,date_trp,motif,nom_b,compte_banc,montant from trpaiement join boutiques using(id_bo)                                                                                          
                                      ...
                                      where extract(day from date_trp)=extract(day from '$_POST[dater]'::date)
                                      and extract(month from date_trp)=extract(month from '$_POST[dater]'::date)
                                      and extract(year from date_trp)=extract(year from '$_POST[dater]'::date)
                                      order by nom_bo
													");
echo '<table align="center" style="width:70%;" class="jquery.ui-widget ui-widget-content myborder" cellpadding="8" cellspacing="0" rules="rows">';
...
while ($lignebo2=pg_fetch_assoc($resultatbo2))

 {
	...
	echo '<td>'.$lignebo2['date_trp'].'</td>';
	echo '<td>'.$lignebo2['motif'].'</td>';
	...
	echo '<tr class="header2 lgauche bw">';
			echo '<td colspan="7" align="center"><strong>TOTAL</strong></td>';
			echo '<td colspan="2" class="ldroite cnoire"><strong>'.number_format($som_t2bo,0,' ',' ').'</strong><sup>F</sup></td>'; 
			echo '</tr>';
}
}
echo '</table><br>';
...
where extract(year from date_dep)=extract(year from '$_POST[dater]'::date)
and extract(month from date_dep)=extract(month from '$_POST[dater]'::date)
and extract(day from date_dep)=extract(day from '$_POST[dater]'::date)
and id_bo=$uti order by date_dep desc");	
echo '<table align="center" style="width:70%;" class="jquery.ui-widget ui-widget-content myborder" cellpadding="8" cellspacing="0" rules="rows">';			                             	
			echo '<tr class="header3 bw"><th colspan="8"><h5 align="center" class="titre1">DEPENSES EFFECTUEE</h5></th></tr>';
			echo '<tr class="header2 lgauche bw">
			<th>N°</th>
...
while ($ldep=pg_fetch_assoc($depenses))
 {
	...
	echo '<td>'.$ldep['crencier'].'</td>';
	echo '<td>'.$ldep['beneficiere'].'</td>';
...
$verait=pg_query($connexion, "SELECT verait.*, nom_b,compte_banc 
											...
											where extract(day from date_vr)=extract(day from '$_POST[dater]'::date)
											and extract(month from date_vr)=extract(month from '$_POST[dater]'::date) 
											and extract(year from date_vr)=extract(year from '$_POST[dater]'::date)
