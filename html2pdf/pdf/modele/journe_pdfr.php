...
$dater=isset($_GET['dater'])?$_GET['dater']:$_POST['dater'];
//$dater=$_GET['dater'];
...
<table class="w90 b0" cellspacing="0mm" cellpadding="0" align="center">
		<tr><td class="myalc w1 cl3 t2 bg1">TRACE JOURNALIER LE <?php echo   $_GET['dater']. ' / ' .date('d-M-Y H:i:s')?></td></tr>
</table><br/>
	<table id="data" cellspacing="0.2mm" class="bg2 w100" align="center" rules="cols"> 
		...
		$lsclient=pg_query($connexion, "select distinct id_cl,nom_cl, prenom_cl 
                                    ...
                                    and extract(day from date_fac)=extract(day from '$dater'::date) 
                                    and extract(month from date_fac)=extract(month  from '$dater'::date) 
                                    and extract(year from date_fac)=extract(year from '$dater'::date) order by nom_cl, prenom_cl");
...
while ($ligne=pg_fetch_assoc($lsclient))
 ...
 $lscountcl=pg_query($connexion, "select count(*) as nb from (select id_cl,id_ar,prix_vente  
                              ...
                              where extract(day from date_fac)=extract(day from '$dater'::date) 
                              and extract(month from date_fac)=extract(month  from '$dater'::date) 
                              and extract(year from date_fac)=extract(year from '$dater'::date)  
	...
	$lsachat=pg_query($connexion, "select id_cl,lib_ar, type_ar, sum(qte_ar) as qte_ar,facture_con.prix_vente,sum(qte_ar*facture_con.prix_vente) as montant 
	                           ...
	                           where extract(day from date_fac)=extract(day from '$dater'::date) 
                              and extract(month from date_fac)=extract(month  from '$dater'::date) 
                              and extract(year from date_fac)=extract(year from '$dater'::date) and id_cl=$id_cl 
	...
	echo '<tr>
		...
		$lsagence=pg_query($connexion, "select distinct id_bo,nom_bo 
                                    ...
                                    where  
                                    extract(day from date_tr)=extract(day from '$dater'::date) 
                                    and extract(month from date_tr)=extract(month  from '$dater'::date) 
                                    and extract(year from date_tr)=extract(year from '$dater'::date) order by nom_bo");
...
while ($ligne=pg_fetch_assoc($lsagence))
 ...
 $lscountbo=pg_query($connexion, "select id_bo,count(*) as nb from (select id_bo  
                              ...
                              where extract(day from date_tr)=extract(day from '$dater'::date) 
                              and extract(month from date_tr)=extract(month  from '$dater'::date) 
                              and extract(year from date_tr)=extract(year from '$dater'::date) order by id_bo) as tt 
	...
	$lsachat2=pg_query($connexion, "select id_bo,lib_ar, type_ar, qte_ar,transferts_con.prix_vente,(qte_ar*transferts_con.prix_vente) as montant 
	                           ...
	                           where extract(day from date_tr)=extract(day from '$dater'::date) 
                              and extract(month from date_tr)=extract(month  from '$dater'::date) 
                              and extract(year from date_tr)=extract(year from '$dater'::date) and id_bo=$id_bo 
...
(
     (
       select id_ar,sum( qt_ar) as entree 
        ...
        where  id_bo=1
        and extract(day from date_1)=extract(day from '$dater'::date)
        and extract(month from date_1)=extract(month from '$dater'::date) 
...
$reproduit=pg_query($connexion, "select id_ar,lib_ar, type_ar,sum( qt_ar) as qte_ar 
                                         ...
                                         and extract(day from date_1)=extract(day from '$dater'::date)
                                         and extract(month from date_1)=extract(month  from '$dater'::date)
                                         and extract(year from date_1)=extract(year from '$dater'::date) 
...
<!-- ==========================================================VERSEMENTS-CLIENTS=================================== -->
		...
		$resultatcl2=pg_query($connexion, "SELECT facpaiement.id_cl,id_fac,nom_cl,prenom_cl,date_facp,motif,montant from facpaiement join clients using(id_cl) 
													...
													and extract(day from date_facp)=extract(day from '$dater'::date)
													and extract(month from date_facp)=extract(month  from '$dater'::date)
													and extract(year from date_facp)=extract(year from '$dater'::date)
													order by date_facp
													");
		$i=1;
...
<!-- ==========================================================VERSEMENTS-AGENCES=================================== -->
		<table id="data" cellspacing="0.2mm" class="bg2 w100" align="center" rules="cols"> 
		<tr class="header3">
		    <th style="text-align: center;" colspan="8" >VERSEMENTS EFFECTUEE PAR DES AGENCES</th>	
		</tr>
		<tr class="header3">
			<th style="width: 4px;text-align: center;"></th><th style="width: 25%;text-align: left;">AGENCES</th>
			<th style=" width: 15%;text-align: left;">DATE</th>
		...
		<?php
		$resultatbo2=pg_query($connexion, "SELECT id_tr,id_bo,nom_bo,date_trp,motif,montant,nom_b,compte_banc 
		from trpaiement join boutiques using(id_bo) 
full join banques on trpaiement.id_b=banques.id_b where extract(day from date_trp)=extract(day from '$dater'::date)
and extract(month from date_trp)=extract(month  from '$dater'::date)
and extract(year from date_trp)=extract(year from '$dater'::date)
order by date_trp");
		$i=1;
while ($lignebo2=pg_fetch_assoc($resultatbo2))

 {
	...
	echo '<td style="text-align: left;">'.$lignebo2['motif'].'</td>';
	...
	echo '<tr>';
			echo '<th colspan="6" align="center" class="prix"><strong>TOTAL</strong></th>';
...
$depenses=pg_query($connexion, "SELECT * from depences 
                                   where extract(year from date_dep)=extract(year from '$dater'::date)
                                   and extract(month from date_dep)=extract(month  from '$dater'::date) 
                                   and extract(day from date_dep)=extract(day from '$dater'::date)
...
while ($ldep=pg_fetch_assoc($depenses))

 {
	...
	echo '<td style="text-align: left;">'.$ldep['beneficiere'].'</td>';
...
$resultat0=pg_query($connexion, "SELECT sum(payee) as payee,sum(reste) as reste,sum(somme) as somme 
													from transferts 
													where extract(year from date_tr)=extract(year from '$dater'::date)  
													and extract(month from date_tr)=extract(month  from '$dater'::date) 
													and extract(day from date_tr)=extract(day from '$dater'::date)
													");
$resultat1=pg_query($connexion, "SELECT sum(payee) as payee,sum(reste) as reste,sum(somme) as somme 
													from facture 
													where extract(month from date_fac)=extract(month  from '$dater'::date) 
													and extract(day from date_fac)=extract(day from '$dater'::date)
													and id_bo=$uti");
$resultat2=pg_query($connexion, "SELECT sum(payee) as payee,sum(reste) as reste,sum(somme) as somme 
													from achat where extract(month from date_1)=extract(month  from '$dater'::date) 
													and extract(day from date_1)=extract(day from '$dater'::date)
													and id_bo=$uti");
$resultat3=pg_query($connexion, "SELECT sum(montant) as montant 
													from depences where extract(month from date_dep)=extract(month  from '$dater'::date)
													and extract(day from date_dep)=extract(day from '$dater'::date)
													and id_bo=$uti");
...
<table id="data" cellspacing="0.2mm" class="bg2 w100" align="center" rules="cols"> 
		...
		$verait=pg_query($connexion, "SELECT verait.*, nom_b,compte_banc 
											...
											where extract(day from date_vr)=extract(day from '$dater'::date) 
											and extract(month from date_vr)=extract(month  from '$dater'::date) 
											and extract(year from date_vr)=extract(year  from '$dater'::date)
