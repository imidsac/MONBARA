...
$EDATE=(date('Y'));
	...
	$vil=pg_query($connexion, "select * from boutiques where id_bo=1");
...
group by lib_ar,type_ar,id_ar order by lib_ar");
 $resultate=pg_query($connexion, "select id_ar,lib_ar,type_ar,entree,sortie, f_stoc_ar(id_ar,1) from
(
     (
...
<td class="myalg cl3 t2"><?php echo $nmois.'/'.$EDATE?></td>
</tr>
</table>
...
where extract(month from date_fac)=$mois and extract(year from date_fac)=f_annee() and facture.id_bo=1
...
<td class="myalc" style="width: 50%;border: 0.1mm #969696;">
<table class="w90 b0" cellspacing="0mm" cellpadding="0" align="center">
		<tr><td class="myalc w1 cl3 t2 bg1">SORTIES</td></tr>
</table><br/>
	<table id="data" cellspacing="0.2mm" class="bg2 w100" align="center" rules="cols"> 
		<tr class="header3">
		   <th style="width: 4px;text-align: center;"></th>
		...
		while ($ligne=pg_fetch_assoc($resultat))
 		{
			...
			echo '<td style="text-align: left;">'.$ligne['lib_ar'].'</td>';
			echo '<td style="text-align: left;">'.$ligne['type_ar'].'</td>';
			echo '<td style="text-align: right; color:#000;">'.number_format($ligne['qte_ar'],0,' ',' ').'</td>';
			echo '</tr>';
			$i++;
		}
		?>
