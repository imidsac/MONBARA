...
$resultat=pg_query($connexion, "SELECT facture.*,(date_fac)::date as dte, nom_cl,prenom_cl,add_cl, tel1_cl from facture join clients using(id_cl)
where extract(year from date_fac)=$annee 
...
where extract(year from date_fac)=$annee 
and extract(month from date_fac)=$mois and id_bo=$uti ");
$resultat1=pg_query($connexion, "SELECT now()::date as dte1");
$datej=pg_fetch_assoc($resultat1);
$rr=pg_fetch_assoc($rmois);
if($rr['nb']!=0) {
	echo '<a href="html2pdf/pdf/tclient.php?&mois='.$mois.'&annee='.$annee.
												'" title="Trace clients Mois: '.getPeriodes($mois).'">
												<img src="images/pdf/pdf.png" width="3%" height="5%" alt="" align="right" border="0" />												
												</a>';
if($_SESSION['gid'] == 3 || $_SESSION['gid'] == 1000 ) {		
echo '<a href="vente_fac_moir.php?&bar='.$bar.'&mois='.$mois.'&annee='.$annee.'">
...
echo '<tr  class="header3 bw">';
		if($_SESSION['gid'] == 3 || $_SESSION['gid'] == 1000 ) {	
			echo '<td colspan="11">
				<a href="ajo_fac.php?&mois='.$mois.'&annee='.$annee.'&bar='.$bar.'">
...
if($_SESSION['gid'] == 3 || $_SESSION['gid'] == 1000 ) {	
echo '<a href="ajo_fac.php?&mois='.$mois.'&annee='.$annee.'&bar='.$bar.'">
				<button id="myb"  class="ui-state-active ui-corner-all boutons">FAIRE UNE VENTE</button>
				</a>';
} 
}
